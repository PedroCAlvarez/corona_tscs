---
title: "CoronaNet COVID-19 Government Response Event Dataset"
header-includes:
   - \usepackage{lineno}
   - \linenumbers
csl: nature.csl
...
overall <- to_plot  %>%
  ...
  ungroup %>% 
    mutate(last_rank = ifelse(date_announced==max(date_announced),`Country Rank`,NA),
         lab_country = ifelse(last_rank %in% c(1,4,14,68,92,125,181,191),as.character(country),
                              NA)) %>% 
  ungroup %>% 
  ggplot(aes(y=med_est,x=date_announced)) +
  #geom_ribbon(aes(ymin=low_est,ymax=high_est,group=country),alpha=0.2) +
...
3.	A set of other policy dimensions:
   ...
   d.	The compliance rules for the policy (E.g. The quarantine used to be voluntary but now its mandatory)
...
As such, to ensure the comprehensiveness of the data, before designing the survey, we collected in depth, over-time data on policy actions taken by one country, Taiwan, since the beginning of the outbreak as well as cross-national data on travel bans implemented by most countries for a total of 245 events. The specific data source we cross referenced for this effort was the March 20, 2020 version of the New York Times article on travel restrictions across the globe [@salcedo2020]. We chose to focus on Taiwan on because of its relative success, as of March 28, 2020, in limiting the negative health consequences of COVID-19 within its borders[@beech2020]. As such, it seemed likely at the time that other countries would choose to emulate some of the policy measures that Taiwan had implemented, which increases the comprehensiveness of the questions we ask in our survey. Indeed at the time of writing, it would appear that some countries have indeed sought to emulate Taiwan's response[@ @aspinwall2020]. 
...
We deeply thank the very large number of research assistants who coded this data. Their affiliations and vita are listed in the appendix.  Our research assistants include:
