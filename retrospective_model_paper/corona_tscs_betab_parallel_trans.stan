...
functions {
  ...
  real partial_sum(int[,] y_slice,
    ...
    for(r in 1:size(y_slice)) {
        ...
        vector[end2 - start2 + 1] prop_infected_trans; // transform it so that it is always increasing
                                                      // by construction
        ...
        // latent infection rate (unobserved), on the logit scale (untransformed)
            ...
            prop_infected = alpha_infect + count_outbreak[start2:end2,1] * poly_nonc1  +
              count_outbreak[start2:end2,2] * poly_nonc2 +
              count_outbreak[start2:end2,3] * poly_nonc3 +
              world_infect*month_cases[start2:end2] +
              Q_supp[start2:end2,1:S]*suppress_effect_raw +
              Q_lock[start2:end2,1:L]*lockdown_effect_raw +
              Q_mob[start2:end2,1:G]*mob_effect_raw;
              
            //need a recursive function to transform to ordered vector
            
            for(i in 1:obs) {
              if(i==1) {
                prop_infected_trans[1] = prop_infected[1];
              } else {
                prop_infected_trans[i] = prop_infected_trans[i-1] + exp(prop_infected[i]);
              }
            }
        ...
        mu_cases = inv_logit(pcr_spec + finding*prop_infected_trans);

        log_prob += normal_lpdf(country_test_raw[s]|mu_test_raw,sigma_test_raw); // more likely near the middle than the ends
        ...
        for(g in 1:G)
          log_prob += normal_lpdf(to_vector(mobility[start2:end2,g])|mob_alpha_const[g] + Q_lock[start2:end2,1:L]*suppress_hier_const_raw[g],sigma_med[g]);
        
        
    mu_tests = inv_logit(alpha_test + 
                      //test_max[start:end] * test_max_par + 
                          test_baseline * prop_infected_trans   +
                          country_test_raw[s] * prop_infected_trans .* lin_counter[start2:end2] +
                          test_lin_counter * lin_counter[start2:end2]);
                              //test_lin_counter * lin_counter[start2:end2] +
                              //test_lin_counter2 * count_outbreak[start2:end2,2] +
                              //country_test_raw2[this_cc] .* mix_prop_std .* count_outbreak[start2:end2,2]);
        ...
        for(n in start2:end2) {
          ...
          if(q>0) {
            ...
            //log_prob += sum(prop_infected[start2:(n-start2+1)]);
          } else {
            // prior implies inflation rate (cases/true infected) is between 2 - 20
            //print("normal prior");
            //print(log_prob);
            if(lin_counter[n-start2+1]<.35) {
