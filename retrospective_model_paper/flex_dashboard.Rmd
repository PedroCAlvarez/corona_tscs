...
```{r statetrend}
  ...
  infldata <- reactive({
    filter(infl_data,state %in% input$state)
  })
...
renderPlot({
  ...
  if(input$present) {
    ...
    ggplot(aes(y=med_est_pres,x=month_day)) +
      geom_ribbon(aes(ymin=low_est_pres,
        ymax=high_est_pres,
      fill=state),alpha=0.5,colour=NA) +
  ...
  out_plot <- out_plot +
  geom_line(aes(linetype=state),colour="black") +
  theme_minimal() +
  ...
  if(input$percap) {
     out_plot <-  out_plot + geom_point(data=infldata(),aes(y=inf_pr),colour="blue",size=5) +
