...
In this document we put forward a model for the rate of infected people $I_{ct} \in (0,1)$ in a given state/region $c$ given the the number of cases $a_{ct}$ and the number of tests $q_{ct}$ for all time periods $t \in T$.
...
We assume that $I_{ct}$ is an unobserved Beta-distributed variable with a 3-order polynomial time trend of the number of post-outbreak time periods $T_O < T$, where an outbreak begins at the first reported case in a given area.
...
\begin{align}
\operatorname{Pr}(I_{ct} \mid T=t) \sim \operatorname{Beta}(&\alpha_1 + \beta_{O1}\sum_{c=1}^{C} \textbf{1}(a_{ct'}>0) \forall t'<t + \beta_{S1}X_{c} + \\
...
The first is a constant factor, $\beta_{S1}$, which is a parameter indicating the strength of state-level suppression measures $X_c$.
This constant parameter is meant to capture the ability of states or regions to stop the spread of transmission due to foreign travelers arriving in the state/region, which occurs at a roughly constant level over time. 
...
The actual false positive rate of the tests being used for the virus are as yet unknown.^[For a discussion, see https://www.realclearpolitics.com/articles/2020/03/18/the_perils_of_mass_coronavirus_testing_142693.html.] For the purposes of this simulation, we will allow $\alpha_3$ to float as it will not affect identification of the sign of the covariates.
...
# Comparison to Observational Model
