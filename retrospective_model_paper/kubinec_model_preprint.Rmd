...
combined <- group_by(combined,state) %>% 
  ...
  filter(!is.na(grocery),!is.na(trendline_extremely_concerned),!is.na(trendline_gotten_worse))
...
world_infect <- arrange(world_infect,month_day) %>% 
  mutate(world_infect=world_infect - coalesce(dplyr::lag(world_infect,n=19),0),
         world_infect=as.numeric(scale(world_infect)))
...
real_data <- list(time_all=length(unique(combined$month_day)),
                 ...
                 count_outbreak=combined$world_infect,
