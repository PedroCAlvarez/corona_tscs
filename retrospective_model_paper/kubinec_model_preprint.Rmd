---
title: "Partisanship and the Spread of COVID-19 in the United States"
author: 
  - Robert Kubinec:
      email: rmk7@nyu.edu
      institute: nyuad
...
> H2: Low COVID-19 infections in areas that voted for Trump cannot be explained by behaviors commonly associated with disease suppression (i.e., social distancing).
...
Because the capacity to test changed significantly over time, we include a linear time interaction (denoted $L_t$) to allow testing capacity to increase.^[For a very compelling visualization of this process with empirical data from the COVID-19 pandemic, we refer the reader to this website: https://ourworldindata.org/grapher/covid-19-tests-cases-scatter-with-comparisons.]
...
\begin{align}
q_{ct} & \sim \operatorname{Beta-Binomial}(c_p, \mu \phi_p, (1-\mu) \phi_p)\\
\mu &= g^{-1}(\alpha_2 + \beta_b I_{ct} + 
\beta_{cq}I_{ct}L_t) + \beta_L L_t)
(\#eq:binom4)
\end{align}
...
\begin{align}
a_{ct} &\sim \operatorname{Beta-Binomial}(q_{ct}, \mu \phi_a, (1 - \mu) \phi_a) \\
\mu &= g^{-1}(\alpha_3 + \beta_a I_{ct}).
(\#eq:binom5)
\end{align}

where $I_{ct}$ is now equal to the linear model vector $\mu$ shown in \@ref(eq:binom).
...
We also extend this model in order to analyze the mediation of a subset of covariates $X'_{ct}$ by adding a mediator $M_{ct}$ to the causal diagram, as in Figure \ref{tikzfig2}. Because the mediator operates on the latent variable, it can allow us to separate the direct from indirect effects of the covariate on the infection rate, which is of particular interest for whether state policies have been mediated through mobility, or the ability of state lockdown orders to reduce social contact. Adding this step into the model only requires an additional likelihood for the mediator-covariate relationship as in @Yuan2009. With the joint posterior estimates, it is straightforward to decompose the effect of $X'_{ct}$ into indirect and direct effects employing the total derivative of the infection rate from @winship1983. 
\begin{figure}
\label{tikzfig2}
\caption{Directed Acyclic Graph for Latent Infection Rate with Mediator}
\ctikzfig{policy_dag_mediate}
\footnotesize{This figure adds a mediator $M_{ct}$ (mobility data) that mediates the relationship between state policies $X'_{ct}$ and the latent infection rate $I_{ct}$.}
\end{figure}

To add continuous mediation covariates $M_{ct}$, which we describe in the next section as cell phone mobility data, we add the following likelihood to the joint posterior:

\begin{equation}
M_{ct} \sim N(\alpha_m + \beta_m X_{ct},\sigma_m)
(\#eq:mediate)
\end{equation}

as well as include all of $M_{ct}$ as linear predictors in \@ref(eq:binom).
...
To analyze the effect of suppression policies, we use data on counts of social distancing policies, restrictions on mass gatherings, restrictions on businesses, mandatory mask orders, restrictions on government services, and stay-at-home orders from the CoronaNet Government Response dataset [@cheng2020]. For each type of policy, we include a variable representing the count of policies in that category effective for a particular day. For each update to an existing policy, we code it as +1 if the update increases the scope of the policy or -1 if it decreases the scope of the policy (down to a minimum of 0). We then interact these policy counts with a linear trend to examine time-varying policy effects. We separately include policies designed to increase health resources like personal protective equipment (PPE) and also policies requiring mask use, though we do not examine time-varying effects of these covariates.
...
We first examine model performance to see how well the model is able to reproduce the observed data. While this kind of predictive validity is only partially useful given that we are interested in a latent quantity, it is still helpful to know whether the model is able to reproduce the empirical distribution. The results of estimating the posterior predictive distribution of cases and tests, drawn from posterior values for the beta-binomial distribution, are shown relative to the original observed values in Figure \@ref(fig:infectscaled) for five states. The plots show that although there is noise in the predictions (represented by the black shaded region), the model is generally able to capture the empirical values (represented by a red line) with high probability. It is important to note as well that the predictions are not always as accurate near the end of the sample time series. Because the model is not attempting to predict future time points, it is generally most accurate near the middle of the series. A tighter fit between the model's predictions and the observed data, on the other hand, would be more useful for predictive validity but at the cost of increased bias due to over-fitting (i.e., the well-known bias/variance trade-off). As Figure \@ref(fig:infectscaled) shows, the model likely under-fits the data as the aim is interpretation and explanation, not forecasting.
...
```{r stateplot,fig.cap="Average Cumulative Count of Infected People by U.S. State as of April 20th",echo=F,fig.asp=0.8}
require(ggrepel)
...
```{r tpi,fig.asp=.8,fig.cap="Measuring States' Testing Rates Relative to Infection Rates"}
...
We next explore the effect of covariates on the latent infection rate. In Figure \@ref(fig:mobility) we show the marginal effect of a 1-SD increase in different types of Google mobility on the infection rate expressed as a fraction of a state's population. In line with the growing research on cellphone mobility and the epidemic, there are strong positive effects of some types of mobility on the spread of the disease, especially grocery stores, retail stores and to a lesser extent residential mobility and transit. Workplace mobility has an estimated zero association with infections, while mobility related to parks has a small but negative association with COVID-19. While these results are somewhat surprising given how large the grocery store effect is relative to the other series, other results confirm with prior suspicions that outdoor activities like attending parks are relatively low-risk for COVID exposure. It should be noted as well that all the effects are estimated simultaneously and the variables are correlated, so the effect of workplace mobility could be partially masked by the other mobility measures. In any case, it is clear that the mobility measures have the single largest estimated effect on suppressing the disease for a time-varying covariate, reaching up to a 0.0219% increase in population infected for a 1-SD increase in grocery store mobility. 
...
Figure \@ref(fig:suppress) shows the marginal effect of all other covariates in the model on the latent infection rate, expressed as the marginal increase in the proportion infected for a standard deviation increase in the covariate. The estimates are further broken out in terms of mediation, and the indirect effects are calculated by summing individual mediation effects across mobility data types. The direct effects and indirect effects are plotted together in panel A, while the total effects (direct effects plus indirect effects) are shown below in panel B. 
...
The use of mediation analysis shows substantial heterogeneity in the types of associations and whether direct and indirect effects tend to complement or substitute each other. First, it is important to note that the single strongest associations in panel B come from the YouGov mask-wearing poll, the Civiqs concern over coronavirus poll, the relative vote share for Donald Trump in 2016, and the percent of a state's residents that are foreign-born. The percentage of foreign born is the single largest factor behind a state's infection rate, with a one SD increase in foreign born associated with a 0.15% percent increase in a state's population that is infected by COVID-19. As these are sample average marginal effects, that number reflects what an *average* state might experience; the effect could well be larger for states with higher infection rates than average. By contrast, the other large effects are strongly negative, with states with more mask-wearers, states with more people concerned over the coronavirus, and states with more people who voted for Donald Trump all experiencing substantially fewer infections, approximately 0.01% of an average state's population. While these effects may seem small, considering that they accumulate over time and that the state with the highest infection rate is 5 percent, they represent substantial changes in the count of infected individuals.

In general, we have more reason to think that the time-varying variables, such as the COVID poll and the Trump approval poll, are likely causally identified relative to variables that only vary by state, such as the share of foreign-born in a state. On the one hand, it is very interesting to note that the share of foreign-born is associated with both indirect and direct effects on the disease. However, considering that the share of foreign-born was fixed before the outbreak, we have limited ability to know why these associations are so strong. However, we have reason to think that the share of foreign-born could be a proxy for international travel and hence may be estimating the long influence of the outbreak of the epidemic in states with easy access to international travel. In fact, this connection was the original motivation for including this variable as a control covariate. At the same time, it is somewhat more difficult to explain why the share of foreign-born would be so strongly related to mobility data, and this is an issue we will leave to future inquiry.

In line with other research, we find that social justice protests had little if any association with infection rates. Though the total effect is positive and statistically significant (i.e., the posterior interval does not include zero), the sample average marginal effect is approximately +`r round(over_all_sum2$med_est[12]*100,5)`%, or small to the point of being minute. There is some evidence, as @protest2020 suggest, that the positive effect of the protests was offset by reduced mobility by non-protesters as the indirect effect is negative, though it is small (`r round(over_all_sum2$med_indirect_est[12]*100,5)`%). As such, it is possible that some additional people were infected, but the number would be so small that it cannot constitute a major outbreak or increase in the level of the epidemic.

Other factors that seem to have little effect include state-level policies designed to increase access to test (probably because they were widely implemented across states early in the pandemic), the level of PM$_{2.5}$ in the atmosphere, and the percent of smokers in a state's population. Some factors also have moderate negative associations the disease, including state per capita income (GDP), concern over the economy and the percentage of youth in a population. 

While it is difficult to generalize given the variety of associations, it does seem possible to rule out some possibilities. The number of young people does not (yet) appear to be strongly positively related to the disease despite accusations that younger people tend to ignore social distancing and other policies. A state's wealth does insulate it somewhat from infections, and states with more people who wear masks and who are concerned about COVID-19 also (perhaps counter-intuitively) see fewer infections over time. Health-related variables including the prior level of public health funding and the number of providers are negatively associated with the disease, though not at a level we might expect compared to other covariates.

In fact, the strongest time-varying factors present in the model concern individual behavior more than policies or state preparedness. Considering that the percentage of foreign residents and Trump vote share were determined long before COVID-19 arrived, it is clear that individuals' willingness to wear masks and their personal fear of the disease is strongly associated with repressing it. In fact, the effect of individual reported mask-wearing is far stronger than the effect of having a state-level mask policy, suggesting again substantial heterogeneity in how state policies are interpreted and applied on the ground. 

At the same time, it is important to note that there are potential post-treatment issues in evaluating both concern over COVID and state policies simultaneously as some have hypothesized that state policies may effect behavior by increasing people's subjective perception of risk [@dryhurst2020risk]. As such, including these variables may deflate the predictive value of other covariates that are causally prior. While we note this issue, as examining risk perceptions is not the main focus of our paper, we do not further explore this particular issue besides noting that the direct effect estimates are likely conservative given that additional mediation may be occurring.

It is interesting finally to note differences in indirect and direct effects of the covariates in panel A of Figure \@ref(fig:suppress). The large effect from the COVID poll primarily comes from mobility data; people who are more concerned about COVID are less likely to frequent places where they could contract the disease. Somewhat counter-intuitively, the mask poll is also associated with repressing COVID through mobility, which we might not expect given that masks can be worn while frequenting locations. The other factor with surprising heterogeneity is per capita income: wealthier states have a strong suppressive *direct* effect on the disease, but the effect mediated through mobility is slightly positive. Wealthier states lower their risk of exposure through some other channel than reducing mobility. A similar trend can be seen with public health funding, which has a moderate negative association with COVID-19 primarily as a direct effect, not through mobility. Again, given that GDP per capita and public health funding do not vary with time, we only have limited ability to say what underlies the association.

Given the prominence of Trump-related variables in explaining the spread of the disease, in addition to the importance of the question to the study of partisanship, we explore the interaction between Trump vote share and Trump approval polls in Figure \@ref(fig:trumpint). In this figure, the effect of Trump approval on COVID-19 is plotted conditional on the vote share within a given state for Trump in 2016 on the $x$ axis. The effects are shown aggregated in panel A and disaggregated across mobility types in panel B. Panel A shows that in general, the effect of partisanship for Trump is largely a direct effect, and is highly conditional on the prior level of votes for Trump. In states with high Trump vote share, increasing Trump approval is associated with less COVID-19. By contrast, in states with low vote share, increasing Trump approval is associated with *more* COVID-19. These effects are quite large: a 1-SD increase in Trump's approval within a state is associated with a -0.02% decline in COVID infections in pro-Trump states and a +0.02% increase in COVID infections in anti-Trump states. 

These effects are difficult to generalize though it is clear that it is primarily a direct effect, not mediated through mobility. However, to the extent that there is a relationship with mobility, it is also negative for both pro and anti-Trump states. It is difficult to assign a precise interpretation to this heterogeneity because of a composition problem: we do not know who exactly is changing their views about Trump at a given time point, and also what other variables might mediate the relationship. We know the effect is direct in the sense that it is not mediated by mobility, but there could be other factors that mediate the relationship which we have not identified. We do note that many plausible factors, such as mask-wearing, state-level policies, and health variables have all been accounted for in the model.

What we can say is that this finding points to very strong associations between partisanship and the spread of the COVID-19, but not via behaviors that we know to suppress the epidemic. States with higher Trump vote shares have seen significantly fewer infections; when Trump approval increases in these states they generally observe fewer infections, which cannot be explained through decreased mobility. Our contention, as expressed through our hypotheses, is that the evidence suggests this relationship is spurious. After all, it is well-known that the early states that were infected with COVID tended to vote against Trump, although partisanship is not why they were more vulnerable to COVID initially. We believe that pro-Trump states received fortuitous outcomes by happening to not be on major travel routes from early COVID-19 hot spots, and subsequent state actions were able to contain the disease well enough that rising approval sentiment for Trump became associated with continued suppression. In other words, although it is implausible to suggest that voting for Trump in 2016 protected a state from an early outbreak in any causal sense, the strength of this association has likely contributed to the polarization of the disease along partisan lines.

```{r trumpint,fig.asp=0.8,fig.cap="Marginal Effects of Increasing Within-State Trump Approval Rating Conditional on Trump Vote Share in 2016"}

# do a big grid
# evaluate the total derivative of infections with respect to vote share and approval

if(calc_int) {
  
  # warning: this will take a long time to run, though it is parallelizable & more cores will
  # reduce the time required
  
  trump_grid <- expand.grid(seq(min(just_data$trump),
                                  max(just_data$trump),length.out = 100),
                             seq(min(just_data$trendline_approve),
                                  max(just_data$trendline_approve),length.out = 100))

    over_vote_share <- parallel::mclapply(1:nrow(trump_grid),
                              function(i) {
      ...
      # with respect to approval rating and vote share
      
      de <- (suppress_effect$estimate[suppress_effect$variable==15] +
        suppress_effect$estimate[suppress_effect$variable==9] * p) + 
        (suppress_effect$estimate[suppress_effect$variable==1] +
        suppress_effect$estimate[suppress_effect$variable==9] * a) 
      
      # loop over mobility
      
      mob_mats <- lapply(1:max(suppress_mob_effect$mobility), function(m) {
        
        ide <- (suppress_mob_effect$estimate[suppress_mob_effect$variable==15 & suppress_mob_effect$mobility==m] +
                  suppress_mob_effect$estimate[suppress_mob_effect$variable==9 & suppress_mob_effect$mobility==m] * p +
                  suppress_mob_effect$estimate[suppress_mob_effect$variable==1 & suppress_mob_effect$mobility==m] +
                  suppress_mob_effect$estimate[suppress_mob_effect$variable==9 & suppress_mob_effect$mobility==m] * a) * mob_effect[,m]
      
    }) 
      
      fear_id <- which(colnames(real_data$suppress)=="trendline_extremely_concerned")
      
      fear <- (suppress_fear_effect$estimate[suppress_fear_effect$variable==15] +
               suppress_fear_effect$estimate[suppress_fear_effect$variable==9]*p +
                 suppress_fear_effect$estimate[suppress_fear_effect$variable==1] +
               suppress_fear_effect$estimate[suppress_fear_effect$variable==9]*a)  * suppress_effect$estimate[suppress_effect$variable==fear_id]
      
      # fear -> mobility pathway
      
      mob_mats_fear <- lapply(1:max(suppress_mob_effect$mobility), function(m) {

        ide <- suppress_mob_effect$estimate[suppress_mob_effect$variable==fear_id & suppress_mob_effect$mobility==m] * mob_effect[,m] * (suppress_fear_effect$estimate[suppress_fear_effect$variable==15] +
               suppress_fear_effect$estimate[suppress_fear_effect$variable==9]*p +
                 suppress_fear_effect$estimate[suppress_fear_effect$variable==1] +
               suppress_fear_effect$estimate[suppress_fear_effect$variable==9]*a)
      ...
      tibble(`Direct Effect`=state_means(de*t(p_infected1)*t(p_infected2)),
             ...
             `Indirect\nMobility` = state_means((Reduce('+', mob_mats) + Reduce('+', mob_mats_fear))*t(p_infected1)*t(p_infected2)),
             ...
             `Within-State Approval Rating`=mean(combined$trendline_approve) + a*sd(combined$trendline_approve))
    
      },mc.cores=8) %>% bind_rows
    
    saveRDS(over_vote_share,"over_vote_share.rds")
} else {
  over_vote_share <- readRDS("over_vote_share.rds")
}

trump1 <- over_vote_share %>% 
    gather(key="Type",value="estimate",-`2016 Trump Vote Share`,
           -`Within-State Approval Rating`) %>% 
    group_by(Type,`2016 Trump Vote Share`,`Within-State Approval Rating`) %>% 
    summarize(med_est=median(estimate),
              high_est=quantile(estimate,.95),
              low_est=quantile(estimate,.05)) %>% 
    filter(Type %in% c("Indirect\nMobility",
                       "Indirect\nCOVID Poll")) %>% 
    ggplot(aes(y=`Within-State Approval Rating`,x=`2016 Trump Vote Share`)) +
    geom_tile(aes(fill=med_est),width=.5) +
    facet_wrap(~Type) + scale_fill_viridis_c() + 
...
trump2 <- over_vote_share %>% 
    ...
    summarize(med_est=median(estimate),
              high_est=quantile(estimate,.95),
              low_est=quantile(estimate,.05)) %>% 
    filter(Type %in% c("Direct Effect",
                       "Total Effect")) %>% 
    ggplot(aes(y=`Within-State Approval Rating`,x=`2016 Trump Vote Share`)) +
    geom_tile(aes(fill=med_est),width=.5) +
    facet_wrap(~Type) + scale_fill_viridis_c() + 
