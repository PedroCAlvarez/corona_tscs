...
test_inf <- lapply(unique(real_data$cc), function(c) {
  ...
  prop_infected_mat_scale[real_data$cc==c,,drop=F] * country_nonc[,c] * real_data$lin_counter[real_data$cc==c] 
}) %>% do.call(rbind,.)
...
tests_pred <- lapply(1:ncol(test_inf), function(i) {
    ...
    # mu_tests <- plogis(alpha[i,1] + test_inf[,i] + combined$test_max * test_max_par[i,])
        ...
        mu_tests <- plogis(alpha_test[i,] + 
                             ...
                             test_inf[,i] +
                             test_max_par[i,]*real_data$test_max +
                             test_lin[i,] * real_data$lin_counter)
  
    tibble(out_pred=extraDistr::rbbinom(n=rep(1,length(mu_tests)),
...
tests_pred %>% 
  ...
  geom_line(aes(y=tests),colour="red",linetype=2) +
...
cases_pred %>% 
  ...
  ggplot(aes(y=out_pred,x=month_day)) +
  geom_line(aes(group=iter),alpha=0.5) +
  geom_line(aes(y=cases),colour="red",linetype=2) +
  facet_wrap(~state,scales="free_y")
...
indirect <- over_med_agg %>% 
  ...
  geom_line(linetype=2) +
  geom_hline(yintercept = 0) +
    facet_wrap(~mobility+lockdown,scales="free_x") +
  xlab("") +
  ylab("") +
  scale_y_continuous(labels=scales::percent)
...
this_infect1 <- filter(ungroup(all_est_state),month_day==max(month_day)) %>% 
  select(state,estimate) %>% 
  group_by(state) %>% 
  mutate(estimate=plogis(estimate),iter=1:n()) %>% 
 ...
 this_infect2 <- filter(ungroup(all_est_state),month_day==max(month_day)) %>% 
  select(state,estimate) %>% 
  group_by(state) %>% 
  mutate(estimate=plogis(estimate)+.001,iter=1:n()) %>% 
  spread(key="state",value="estimate") %>% 
  ungroup %>% 
  select(-iter) %>% 
  as.matrix
...
over_states <- lapply(1:real_data$num_country, function(s) {
  tibble(estimate2=plogis(alpha_test$alpha_test + (mu_test[,1] + sigma_test[,1]*country_test[,s]) * this_infect2[,s] +
                            test_base * this_infect2[,s] + test_lin +
                            test_max_par$test_max_par*this_test_max$test_max[s]),
         estimate1=plogis(alpha_test$alpha_test + (mu_test[,1] + sigma_test[,1]*country_test[,s]) * this_infect1[,s] +
                                                         test_base * this_infect1[,s] + 
                             test_lin + 
                             test_max_par$test_max_par*this_test_max$test_max[s]),
         state_num=s)
}) %>% bind_rows
...
over_states %>%
  ...
  group_by(state) %>%
    summarize(med_est=quantile(estimate2*1000 - estimate1*1000,.5),
            high_est=quantile(estimate2*1000 - estimate1*1000,.95),
            low_est=quantile(estimate2*1000 - estimate1*1000,.05)) %>% 
  ...
  labs(caption = stringr::str_wrap("Figure shows the average number of additional people tested in a given state for an additional 1 person out of a thousand who becomes infected. Estimate is an average and thus may not be accurate for the most recent time period.")) +
  ylab("How Many People Would Be Tested If 1 Additional Resident Was Infected (Per 1,000)")
