...
combined <- group_by(combined,state) %>% 
  ...
  mutate(lockdown=as.numeric(month_day>stayhome & month_day<end_stayhome),
         ...
         lockdown=dplyr::lag(lockdown,n=14)) %>% 
  mutate_at(c("grocery",
              "parks",
              "residential",
              "retail",
              "transit",
              "workplaces"), ~dplyr::lag(.,n=14)) %>% 
    ungroup %>% 
  mutate_at(c("grocery",
              "parks",
              "residential",
              ...
              "Public Health Funding"), ~as.numeric(scale(.))) %>% 
  mutate_at(c("lockdown",
              "emer_outbreak",
              "lockdown_outbreak"),~scale(.,scale=F)) %>% 
...
real_data <- list(time_all=ncol(cases_matrix_num),
                 ...
                 G=ncol(covs_mob),
                 L=ncol(covs_lock),
                 country_pop=floor(unique(just_data$state_pop)/100),
...
if(run_model) {
  ...
  us_fit_scale <- sampling(pan_model_scale,data=real_data,chains=3,cores=3,iter=800,warmup=500,
                           init=0,control=list(max_treedepth=12))
...
calc_sum <- all_est_state %>% 
  ...
  summarize(us_total=sum(estimate)) %>%
  # summarize(us_total=estimate) %>% 
  # group_by(iter) %>% 
...
all_est_state <- as.data.frame(us_fit_scale,"num_infected_high") %>% 
  ...
  group_by(variable) %>% 
  mutate(state_num=as.numeric(str_extract(variable,"(?<=\\[)[1-9][0-9]?0?")),
         time_point=as.numeric(str_extract(variable,"[0-9]+(?=\\])")),
         time_point=ymd(min(combined$month_day)) + days(time_point-1))
...
calc_sum_state <- all_est_state %>% 
  ...
  group_by(time_point,state,suppress_measures,cum_sum_cases) %>% 
...
if(run_model) {
    ...
    over_med <- lapply(1:ncol(covs_mob), function(m) {
      ...
      over_mod <- lapply(seq(min(combined$lockdown_outbreak),max(combined$lockdown_outbreak),
          ...
          tibble(direct_eff=rowMeans((direct_effect[,12] + direct_effect[,13]*d)*p_infected),
                  indirect_eff=rowMeans((mob_effect[,m]*(lock_effect[,m,3] + lock_effect[,m,4]*d)*p_infected)),
