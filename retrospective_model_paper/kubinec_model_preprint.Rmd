...
goog_mobile <- covdata::google_mobility %>% filter(sub_region_1 %in% merge_names$state,
                                                   is.na(sub_region_2)) %>% 
  rename(state=sub_region_1) %>% 
  spread(key = "type",value = "pct_diff")
...
combined <- left_join(combined,select(goog_mobile,state,month_day="date",grocery:workplaces))
...
combined <- group_by(combined,state) %>% 
  ...
  filter(state %in% c("New York","West Virginia","California","Maryland","Hawaii"),
         !is.na(grocery))
...
mobility <- select(ungroup(combined),month_day,state,grocery:workplaces) %>% arrange(state,month_day)
