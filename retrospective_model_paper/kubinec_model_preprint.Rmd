...
# add protest data
# need to impute
prot_data <- read_csv("../data/simulation/protest/count_love/count_love_data.csv") %>% 
  select(event=`Event (legacy; see tags)`,
         articles=`Total Articles`,
         state=`State Name`,
         state_code=`State Code`,everything()) %>% 
  mutate(floyd=as.numeric(grepl(x=Tags,pattern="racial justice") & Date>ymd("2020-05-25"))) %>% 
  missRanger(pmm.k=10L) %>% 
  group_by(state,Date,floyd) %>% 
  summarize(sum_prot=sum(Attendees)) %>% 
  spread(key="floyd",value="sum_prot",fill = 0) %>% 
  select(reg_prot="0",
         floyd_prot="1")
...
approval <- read_csv("../data/simulation/Civiqs/civiqs_approve_president_trump_states_2020-06-24.csv") %>%   select(date,state,approve)
concern <- read_csv("../data/simulation/Civiqs/civiqs_coronavirus_concern_states_2020-06-24.csv") %>% 
  select(state,date,extremely_concerned)
# response <- read_csv("../data/simulation/Civiqs/civiqs_response_local_states_2020-06-24.csv") %>% 
  
economy <- read_csv("../data/simulation/Civiqs/civiqs_economy_us_now_states_2020-06-24.csv") %>% 
  select(date,state,very_good)
...
combined <- left_join(combined,select(prot_data,state,Date,reg_prot,floyd_prot),by=c(month_day="Date",
                                                                           "state")) %>% 
  mutate(reg_prot=coalesce(reg_prot,0),
         reg_prot=reg_prot/state_pop,
         floyd_prot=coalesce(floyd_prot,0),
         floyd_prot=floyd_prot/state_pop)
...
combined <- group_by(combined,state) %>% 
  ...
  ungroup %>%
  group_by(month_day) %>% 
  mutate(world_infect=sum(Difference)) %>% 
  group_by(state) %>% 
  arrange(state,month_day) %>% 
  ...
  filter(!is.na(grocery),!is.na(extremely_concerned),!is.na(very_good))
...
time_outbreak_center <- matrix(combined$outbreak_time,byrow = T,
                               nrow=length(unique(combined$state)),
                                                   ncol=length(unique(combined$month_day)))

ortho_time <- poly(scale(1:max(time_outbreak_center)),degree=3)
...
serology <- tibble(state_id=c("Washington",
                              ...
                              "Utah",
                              "Connecticut"),
                   inf_pr=c(48300/4274336,
                                 641800/9261183,
                                 117400/6345945,
                                 161900/6109434,
                                 47400/2174312,
                                 176700/3576923),
                   case_pr=c(4300/4274336,
                                 53800/9261183,
                                 10500/6345945,
                                 6800/6109434,
                                 4500/2174312,
...
# convert to numbers from dates/factors

lev_state <- levels(factor(combined$state))
lev_date <- levels(factor(combined$month_day))

serology <- mutate(serology,
                   state_id=as.numeric(factor(state_id,levels=lev_state)),
                   date_begin=as.numeric(factor(date_begin,levels=lev_date)),
                   date_end=as.numeric(factor(date_end,levels=lev_date)))

real_data <- list(time_all=length(unique(combined$month_day)),
                 num_country=length(unique(combined$state)),
                 cc=rep(1:length(unique(combined$state)),each=length(unique(combined$month_day))),
                 ...
                 sero=as.matrix(select(serology,-state_id,-date_end)),
                 sero_time=serology$date_end,
                 sero_country=serology$state_id,
                 maxO=nrow(ortho_time),
                 country_pop=floor(just_data$state_pop),
                 cases=cases_matrix_num[,1],
                 ortho_time=ortho_time,
                 phi_scale=.01,
                 cases_per_cap=combined$cases_per_cap,
                 count_outbreak=rep(scale(time_global$time_global),
                                    times=length(unique(combined$state))),
                 tests=tests_matrix_num[,1],
                 time_outbreak=time_outbreak_center,
                 suppress=covs,
                 mobility=covs_mob,
                 lockdown=covs_lock)
...
if(run_model) {
  ...
  # load some good init values
  
  this_init <- readRDS("this_init.rds")
  
  this_init <- list(this_init,this_init,this_init)
  
  us_fit_scale <- sampling(pan_model_scale,data=real_data,chains=3,
                           cores=3,iter=500,warmup=300,
                           init=this_init,
...
combined <- mutate(ungroup(combined),key=1:n()) %>% 
  ...
  mutate(cum_sum_cases=Difference,
         ...
         lag_case=ifelse(dplyr::lag(cum_sum_cases,n=14)<0 & !is.na(dplyr::lag(cum_sum_cases,n=20)),0,
                         coalesce(dplyr::lag(cum_sum_cases,n=20),0)),
         cum_sum_cases = cum_sum_cases - lag_case)

all_est_state <- as.data.frame(us_fit_scale,"prop_infected") %>% 
  mutate(iter=1:n()) %>% 
  gather(key="variable",value="estimate",-iter) %>%
  mutate(key=as.numeric(str_extract(variable,"[0-9]+"))) %>% 
  left_join(select(combined,deaths,recovered,month_day,state,key,cum_sum_cases,state_pop),by="key")
...
calc_sum_state <- all_est_state %>% 
  ...
  mutate(cum_est=cumsum(estimate) - recovered - deaths,
         cum_est=cum_est-coalesce(dplyr::lag(cum_est,n=19),0),
  cum_sum_cases = cum_sum_cases / (state_pop)) %>% 
  group_by(month_day,state,cum_sum_cases) %>% 
  summarize(med_est=quantile(cum_est,.5),
            high_est=quantile(cum_est,.95),
            low_est=quantile(cum_est,.05)) 
...
if(run_model) {
    ...
    over_all2 <- lapply(1:max(suppress_effect$variable), function(s) {
        ...
        this_eff <- (suppress_effect$estimate[suppress_effect$variable==s]*p_infected)/100 
...
test_var <- as.data.frame(us_fit_scale,"country_test_raw") %>%
  mutate(iter=1:n()) %>%
  gather(key="variable",value="estimate",-iter) %>%
  mutate(state=as.numeric(str_extract(variable,"(?<=\\[)[1-9][0-9]?0?")))

this_infect <- filter(ungroup(num_infected),month_day==max(month_day)) %>% 
  left_join(tibble(state=1:nrow(cases_matrix),
...
this_test_max <- ungroup(combined) %>% 
  group_by(state) %>% 
  summarize(max_test=max((tests/(state_pop/100))))

test_var <- left_join(test_var,tibble(state=1:nrow(cases_matrix),
                                                state=cases_matrix$state,
                                                state_pop=real_data$country_pop)) %>% 
