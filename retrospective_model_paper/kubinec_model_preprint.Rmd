...
combined <- group_by(combined,state) %>% 
  ...
  ungroup 
  # # if any times that cumulative cases went down, take the average
  # mutate(Difference=ifelse(Difference<0,(lag(Difference) + lead(Difference))/2,Difference),
  #        tests=ifelse(tests<0,(lag(tests) + lead(tests))/2,tests))

# need to fix an issue with Georgia for 4/12 (decrease in cumulative cases ???) and Idaho
# data anomalies

# combined <- mutate(combined,Difference=ifelse(state=="Georgia" & month_day==ymd("2020-04-12") & Difference<0,(Difference[state=="Georgia" & month_day==ymd("2020-04-11")] + Difference[state=="Georgia" & month_day==ymd("2020-04-13")])/2,Difference),
#                    Difference=ifelse(state=="Idaho" & month_day==ymd("2020-05-03") & Difference<0,(Difference[state=="Idaho" & month_day==ymd("2020-05-02")] + Difference[state=="Idaho" & month_day==ymd("2020-05-04")])/2,Difference))
          # tests=case_when(state=="Vermont" & month_day==ymd("2020-05-11")~floor(tests[])
          #          recovered=coalesce(recovered,0),
          #          weekend=wday(month_day) %in% c(1,7)) 
  # group_by(state) %>% 
  # arrange(month_day) %>% 
  # mutate(roll_case=roll_mean(Difference,n=3,fill=0),
  #        roll_test=roll_mean(tests,n=3,fill=0),
  #        Difference=ifelse(weekend,floor(roll_case),Difference),
  #        tests=ifelse(weekend,floor(tests),tests)) %>% 
...
if(run_model) {
  ...
  us_fit <- sampling(pan_model,data=real_data,chains=4,cores=3,iter=800,warmup=500,init=init_vals)
...
case_count <- left_join(case_count,select(combined,deaths,recovered,month_day,state),
  ...
  mutate(cum_sum_cases = cum_sum_cases - deaths - recovered,
...
In addition to the estimation of covariates, the model provides further useful information by parameterizing the relationship between the unobserved infection rate and the number of tests conducted in a given state. These individual parameters are shown in Figure \@ref(fig:tpi). The scale of the y axis shows how much tests will increase for a unit increase of the infection rate on the logit scale. Given the multiplicative nature of the logit scale, the best way to understand the parameter is given a particular infection rate. For example, if we use a recent estimate of the US population at 330 million, and take the recent observed case count (700,000) as the quite conservative estimate of infected, then we have an infected rate of `r round(.7/330,3)` for the US population. Given a testing parameter of 1.03 and a baseline of equal testing/infection capacity, then a unit increase (logit scale) in the infection rate from `r round(.7/330,3)` to `r plogis(qlogis(.7/330)+1)` would increase the testing rate to `r round(plogis(0.98*(qlogis(.7/330)+1)),3)`. As such, the model provides a relatively nuanced relationship that can help provide insight into how much states are able to increase tests in proportion to infection rates.
...
test_var %>%
  ...
  ylab("Proportion Tested Relative to Proportion Infected (Lower Numbers = Higher Testing Rate)")
