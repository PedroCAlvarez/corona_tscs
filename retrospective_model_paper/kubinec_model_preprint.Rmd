...
combined <- group_by(combined,state) %>% 
  ...
  group_by(state) %>% 
  arrange(state,month_day) %>% 
  mutate(cases_per_cap=Difference/(state_pop/100),
...
combined <- mutate(ungroup(combined),key=1:n()) %>% 
  group_by(state) %>% 
  arrange(state,month_day) %>% 
  mutate(cum_sum_cases=cumsum(Difference),
...
#          month_day=as.numeric(str_extract(variable,"[1-9][0-9]?0?(?=\\])")),
#          month_day=ymd(min(combined$month_day)) + days(month_day-1))
# 
# # iterate this bugger
# 
# 
