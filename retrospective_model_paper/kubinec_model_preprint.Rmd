...
abstract: "In this paper we present a retrospective Bayesian model that is much simpler than epidemiological models of disease progression but is still able to identify the effect of covariates on the historical infection rate. The model is calibrated using serology surveys issued by the Center for Disease Control. To apply the model, we show that as of July 14th, there are approximately 4 million infected people presently infected in the United States, and these people are increasingly concentrated in states with rising approval polls for President Donald Trump in 2016, higher rates of cardiovascular deaths, fewer medical providers, less public health spending and less income per capita than other states. We also show that state policies towards COVID-19 have widely heterogeneous effects in terms of when they were imposed that makes generalization about the success of policies difficult. For these reasons, we argue that more disaggregated measures, such as mobility and survey data, is more useful for understanding the sociopolitical mechanisms behind COVID-19 spread. We find that increased mobility measured by Google cell phones in grocery stores and retail establishments has the highest correlation with subsequent COVID-19 spread.^[To reproduce the model and to access the underlying Stan code, please see our [Github page](https://github.com/saudiwin/corona_tscs). This paper is part of the [CoronaNet project](https://lumesserschmidt.github.io/CoronaNet/) collecting data on government responses to the COVID-19 pandemic. For helpful comments we thank participants of the 2020 Polnet/Politics and Computational Social Science Conference. We thank Tesea Conte, Muhannad AlRamlawi, Shiva Teerdhala, and Luke Burkholder for invaluable research assistance in evaluating policies.]"
---
...
We use this model to measure important associations between U.S. state-level factors and the disease as of July 14, 2020, including the the duration and timing of COVID-19 related policies, a variety of data on state populations, health preparedness and environmental factors; state-level opinion poll measures, and  Google mobility data and protest activity. We find that, when controlling for a range of potential confounding variables, higher vote share for Donald Trump in the 2016 elections is negatively associated with the spread of the virus, while a rising approval polls of President Trump are positively associated with COVID-19. These two partisan variables represent some of the strongest predictors of COVID-19 spread, with larger effect sizes than structural variables like state income per capita, number of health care providers and level of public health funding. We interpret these results as showing the profound connections between partisanship and the duration and extent of COVID-19 in the United States.

In addition, contrary to previous work, we should that protest activity has a small but positive effect on the spread of COVID-19. This association was likely obscured in prior work due to more limited sample sizes, fewer control covariates and a lack of true identification of the latent infection rate. This finding suggests that even outdoor activities with close physical proximity do constitute a risk to the spread of the virus, as many epidemiologists have warned. 

At the same time, we find that policy measures to combat COVID-19 have ambiguous effects on the virus that vary considerably over time and are difficult to generalize. As we discuss in this paper, it is difficult to collapse policy data to those measures which might have the most significant effect on the spread of the virus. Conclusions about the efficacy of policeis are likely to be ephemeral without more data and finer-grained measures, contrary to some existing scholarship arguing for identified effects from policy measures. 

Our study also shows that more disaggregated measures, especially opinion polls and mobility data, offer much more useful measures of the mechanisms behind the spread of the virus. Continued survey work on the correlates of COVID-19 spread combined with mobility measures offers promise for better understanding under what conditions policies may be effective and what role they play in reducing behaviors likely to lead to virus transmission.
...
Given this overview of the intuition behind our approach, we turn to a more formal definition. The outcome to be explained is more specifically defined as the cumulative total of tests and cases reported on a given day. We assume that $I_{ct}$ is an unobserved Beta-distributed variable with a 3-order polynomial time trend of the number of post-outbreak time periods $T_O < T$, where an outbreak begins at the first reported case in a given area. The third-order polynomial reflects the fact that epidemics occur in waves, although the curve is unlikely to be symmetric as a simpler quadratic function would require. We allow the polynomial trends to vary by states hierarchically, i.e., the information about the trends is partially pooled across states. 

The unobserved infection rate $I_{ct}$ is then function of the following parameters:
...
The parameter vector $\beta_O$ represents the effect of independent covariate matrix $X_{ct}$ on the latent infection rate. The world infection parameter measures the spread of the virus due to travel across state borders, and as such will increase rapidly as more states are infected.
...
Because we do not have measures of $I_{ct}$, we need to determine the relationship between $I_{ct}$ and the observed data, tests $q_{ct}$ and cases $a_{ct}$.
First, we can infer that an increasing number of infections is associated with more tests as states try to identify who may have the virus. Furthermore, a rising infection rate is associated with a higher ratio of positive results (reported cases) conditional on the number of tests.
...
To model the number of tests, we assume that each state has an unobserved level of testing parameter, $\beta_{cq}>0$, indicating how strongly each state is willing and able to perform tests as a factor of the unobserved infection rate. We further allow this testing parameter to vary linearly over time as testing capacity increases within states.
The number of observed tests $q_{ct}$ for a given time point $t$ and state $c$ conditional on  the states' population, $c_{p}$, then has a binomial distribution: 

\begin{equation}
q_{ct} \sim \operatorname{Binomial}(c_{p}, g(\alpha_2 + \beta_b I_{ct} + 
...
Given the number of observed tests $q_{ct}$, we can then generate the number of observed cases $a_{ct}$ as a binomial random variable as a proportion of the state population $c_p$:

\begin{equation}
a_{ct} \sim \operatorname{Binomial}(c_p, g(\alpha_3 + \beta_a I_{ct})),
(\#eq:binom3)
\end{equation}
...
\begin{equation}
q_{ct} \sim \operatorname{Beta-Binomial}(c_p, g(\alpha_2 + \beta_b I_{ct} + 
\beta_{cq}I_{ct}L_t) + \beta_L L_t), \phi_q),
(\#eq:binom4)
\end{equation}
...
Three restrictions are necessary to identify the rotation of the latent variable. First, we impose a positivity constraint on the parameter $\beta_a$ so that the latent variable is always increasing in rising case counts, and second, we impose a constraint on the latent variable so that it is always increasing in $t$. 
...
Second, we impose a monotonicity constraint on $I_{ct}$ within each state. Because we know that the count of cases and tests is cumulative, we need to require that $I_{ct}$ is always non-decreasing relative to itself for a given state $c$. We do so by adjusting $I_{ct}$ using the transformation in \@ref(eq:trans):
...
The third and final step in identifying the model is to add on further prior information concerning the location of $I_{ct}$. This information is crucial to identify the scale as opposed to just the rotation of $I_{ct}$. Without further information, $I_{ct}$ will rank the states relative to each other in terms of latent infection rates, but it will not reflect any meaningful scale in terms of percent of the state population. 
...
combined <- group_by(combined,state) %>% 
  ...
  ungroup %>%
  mutate(outbreak_time=outbreak_time/max(outbreak_time)) %>% 
  group_by(state) %>% 
  arrange(state,month_day) %>% 
...
where $s_{ct}$ is the value of seroprevalence at state $s$ and time $t$ and $g^{-1}$ is the logit function. 
...
\mu_q &\sim \operatorname{Normal}(20, 20),\\
...
The only data required to fit the model, in addition to the covariates of interest, are observed cases and tests for COVID-19 by day. In this section, we fit the model to numbers of COVID-19 case counts on US states and territories provided by [The New York Times](https://github.com/nytimes/covid-19-data). By doing so, we can use the differences in trajectories across states to help identify the effect of state-level covariates on the infection rate. We supplement these observed case counts with testing data by day from the [COVID-19 Tracking Project](https://github.com/COVID19Tracking/covid-tracking-data). The testing data starts at March 4th, so we impute the testing data back in time by assuming that the average case/tests ratio stays the same to the origin of the outbreak. Furthermore, as there are discrepancies where the reported number of cases or tests decreases for a given day, we impute these cases and tests through linear interpolation as the number is likely an under/over count of neighboring days.
...
if(run_model) {
  pan_model_scale <- cmdstan_model("corona_tscs_betab_parallel_trans.stan")
...
count_pol %>% group_by(type,month_day,province) %>% summarize(sum_pol=sum(count_pol_eff)) %>% ungroup %>% 
  ...
  ggplot(aes(y=sum_pol,x=month_day)) + geom_line(aes(group=province)) + facet_wrap(~stringr::str_wrap(type,10)) + 
  theme(panel.grid = element_blank(),
        panel.background=element_blank(),
...
We first examine model performance to see how well the model is able to reproduce the observed data. While this kind of predictive validity is only partially useful given that we are interested in a latent quantity, it is still helpful at showing how precise our relatively parsimonious latent variable model is able to capture the empirical cases and tests counts. The results of estimating the posterior predictive distribution of cases and tests, drawn from posterior values for the beta-binomial distribution, are shown relative to the true values in Figure \@ref(infectscaled) for five states. The plots show that although there is noise in the predictions, they are generally able to capture the true values. It is important to note as well that the predictions are not always as accurate near the end of the sample time series. Because the model is not attempting to predict future time points, it is generally most accurate near the middle of the series. A tighter fit between the model's predictions and the observed data, on the other hand, would be more useful for predictive validity but at the cost of increased bias due to over-fitting (i.e., the well-known bias/variance trade-off). As Figure \@ref(infectscaled) shows, the model likely under-fits the data as the aim is interpretation and explanation, not forecasting.
...
t1 <- tests_pred %>% 
  ...
  geom_line(aes(y=tests),colour="red",size=2) +
...
c1 <- cases_pred %>% 
  ...
  ggplot(aes(y=out_pred,x=month_day)) +
  geom_line(aes(group=iter),alpha=0.5) +
  geom_line(aes(y=cases),colour="red",size=2) +
  facet_wrap(~state,scales="free_y") +
    theme(panel.grid = element_blank(),
        panel.background = element_blank()) +
    xlab("") +
...
Given this check on the model's fidelity to the data, we can then report the model's estimates of infected counts for the U.S. population as a whole in \@ref(totalinf). 
...
calc_sum %>% 
  ...
  labs(caption="Blue 5% - 95% HPD intervals show estimated infected and the black line\nshows observed cases from the New York Times.\nThese estimates are based on CDC seroprevalence data and a Bayesian model of how cases and tests are influenced by infection rates.") +
    ...
    over_med <- lapply(1:ncol(covs_mob), function(m) {
      ...
      over_cov <- purrr::map2(lock_covs, int_locks,function(l,li) {
        ...
        over_mod <- lapply(seq(min(just_data[[li]]),max(just_data[[li]]),
          ...
          tibble(direct_eff=rowMeans((direct_effect[,l] + direct_effect[,li]*d)*p_infected1*p_infected2),
                  indirect_eff=rowMeans((mob_effect[,m]*(lock_effect[,m,l] + lock_effect[,m,li]*d)*p_infected1*p_infected2)),
...
indirect <- over_med_agg %>% 
  ...
  geom_line(linetype=2) +
  geom_hline(yintercept = 0) +
    facet_grid(rows=vars(mobility),cols=vars(lockdown),scales="free") +
  xlab("") +
  ylab("") +
  scale_y_continuous(labels=scales::percent) +
  theme(panel.grid=element_blank(),
...
prop_med + indirect  +
      ...
      #                 caption="Results from mediation analysis using MCMC with Stan.\nPanel A shows the proportion of variation attributed to policies that can be explained by mobility data.\nPanel B shows indirect effect of policies mediated through mobility data by number of days after first COVID-19 case detected.") & theme(strip.placement = NULL)
...
suppress_effect <- as.data.frame(us_fit_scale,"suppress_effect") %>% 
  ...
  mutate(variable=as.numeric(str_extract(parameter,"(?<=\\[)[1-9][0-9]?0?")))
    ...
    over_all2 <- lapply(1:max(suppress_effect$variable), function(s) {
        ...
        this_eff <- (suppress_effect$estimate[suppress_effect$variable==s]*t(p_infected1)*t(p_infected2)) 
...
over_all_sum2 <- left_join(over_all_sum2,tibble(variable=1:ncol(covs),
  ...
  mutate(label=recode(label,
                      air="PM 2.5",
                      trendline_approve="Trump\nApproval",
                      mask_wear="Mask Poll",
                      trendline_extremely_concerned="COVID Poll",
                      sum_prot="Social Justice\nProtests",
                      masks="Mask Policies",
                      testing_cap="Testing\nPolicies",
...
over_days %>%
  ...
  xlab("") +
  labs(caption = stringr::str_wrap("Figure shows the average number of additional people tested in a given state for each person who becomes infected. Estimate is an average of the last seven days of data.")) +
