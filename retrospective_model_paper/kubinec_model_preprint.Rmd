...
real_data <- list(time_all=length(unique(combined$month_day)),
                 ...
                 lin_counter=poly(combined$lin_counter,2),
...
test_inf <- lapply(unique(real_data$cc), function(c) {
  ...
  prop_infected_mat_scale[real_data$cc==c,,drop=F] * country_nonc[,c]
    #real_data$lin_counter[real_data$cc==c] 
}) %>% do.call(rbind,.)
...
this_infect <- filter(ungroup(all_est_state),month_day==max(month_day)) %>% 
  mutate(estimate2=plogis(estimate)+.001,
...
over_states <- lapply(unique(test_var$state), function(s) {
  tibble(estimate2=plogis(alpha_test$alpha_test + (mu_test[,1] + sigma_test[,1]*test_var$estimate[test_var$state==s]) * this_infect$estimate2[this_infect$state==s] + test_max_par$test_max_par*test_var$test_max[test_var$state==s]),
         estimate1=plogis((alpha_test$alpha_test + (mu_test[,1] + sigma_test[,1]*test_var$estimate[test_var$state==s]) * this_infect$estimate[this_infect$state==s] +
                                                                                                                                                                                                                    test_max_par$test_max_par*test_var$test_max[test_var$state==s])),
         state=s)
}) %>% bind_rows
...
over_states %>%
  left_join(distinct(combined,state,state_pop),by="state") %>%
  group_by(state) %>%
