...
p2 <- over_all_sum2 %>%
  gather(key="type",value="estimate",matches("direct|indirect")) %>% 
  mutate(typeII=ifelse(grepl(x=type,pattern="indirect"),"Indirect","Direct"),
         typeIII=recode(typeII,Direct="D",Indirect="I"),
         type=str_remove(type,'indirect|direct')) %>% 
...
p3 <- over_all_sum2 %>%
  ggplot(aes(y=med_indirect_est,x=reorder(label,med_indirect_est))) +
  geom_pointrange(aes(ymin=low_indirect_est,ymax=high_indirect_est),alpha=0.8,size=.1) +
  theme_minimal() +
  xlab("")  +
  geom_hline(yintercept=0,linetype=2) +
  geom_text(aes(label=scales::percent(med_indirect_est)),vjust=-1) +
  theme(panel.grid=element_blank(),
        plot.title = element_text(size=12,hjust=0.5),
        strip.text=element_text(face="bold"),
        axis.text=element_text(size=7)) +
    ...
    over_med <- lapply(1:ncol(covs_mob), function(m) {
      ...
      over_cov <- purrr::map2(lock_covs, int_locks,function(l,li) {
        ...
        over_mod <- lapply(seq(min(combined$outbreak_time),
          ...
          tibble(direct_eff=rowMeans(de*p_infected1*p_infected2),
                  indirect_eff=rowMeans(ide*p_infected1*p_infected2),
                 total_eff=rowMeans((de + ide)*p_infected1*p_infected2),
                 prop_med=rowMeans((ide / (de + ide))*p_infected1*p_infected2)) %>% 
...
# prop_med
# 
# ggsave("proportion_mediated.png",width=8,height=6)
