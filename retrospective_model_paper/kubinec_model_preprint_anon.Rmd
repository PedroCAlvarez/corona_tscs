...
The infection and subsequent hospitalization of United States President Donald Trump in early October of 2020 may have signaled the high water mark of a profound difference in how different political camps experienced and responded to the COVID-19 pandemic. While COVID-19 has been a difficult issue for many countries, in the United States profound disagreement over how to respond to the disease has exacerbated long-standing political divisions. However, it is diffult to know how these sociopolitical factors correlate with the spread of the pandemic due to difficult issues in the measurement of the spread of the disease. In this paper, we present a model that is able to directly incorporate biases in COVID-19 testing data, and we use this model to explore in much more detail how the political, social and economic context of COVID-19 influenced the spread of the disease during the first half of 2020. To do so we employ a rich range of adjustment covariates, including daily state-level polling about COVID-19 and the economy and daily Google mobility data.

Although earlier studies have shown that associations between partisanship and COVID-19 exist at the individual level, it is very difficult to obtain inferences at a higher level of aggregation due to well-known biases in disease reporting. To address this critical problem, we present a Bayesian latent variable model to track COVID-19 spread in the United States. The utility of the model, compared to existing approaches, lies in its direct parameterization of the measurement bias stemming from incomplete testing for the disease in the United States. Rather than attempt to use statistical methods to eliminate the bias, we model this structure directly by jointly estimating the effect of covariates on COVID-19 cases and tests via the unobserved infection rate. To identify the model, we employ CDC serology surveys, allowing uncertainty in these samples to propagate into our model estimates while obtaining empirical estimates of the testing bias for calibration purposes.

We use this model to investigate the increasingly clear partisan division over the disease along with public health, economic and policy variables. We provide much more precise estimates of the association between political identity and the disease's spread. We show that Trump's approval ratings and racial justice protests are both associated with rising infections, though the former much more so than the latter. To better explain these associations, we employ mediation analysis in our models to understand how much a particular factor suppresses the disease by reducing mobility (measured through cell phone signals) or by increasing people's concern about the disease (measured through daily polling) as opposed to an unmediated direct pathway. As a result, we show in this paper that areas with high Trump approval tended to see lower infections due to unexplained pathways but experienced more infections through mediated pathways, reducing concern over the disease and increasing mobility.

We make the argument based on this evidence that residents of pro-Trump states wrongly averred that the initial conditions of the epidemic, which was concentrated on the liberal coasts, meant that they could avoid practicing prudent social distancing measures. As a result, people in pro-Trump states have seen more infections despite favorable initial conditions as their approval of the president increased. By contrast, racial justice protests are also associated with increased COVID-19 infections, though the effects are relatively smaller compared to other drivers of the pandemic. Nonetheless, it appears that political mobilization and identity on both the right and the left have undermined adherence to social distancing and other prudent measures to prevent infections.

In addition, we show that the effect of state-level suppression policies targeted at the epidemic varies significantly over the time and exhibits trade-offs in terms of reducing mobility. While stay-at-home orders and business restrictions reduced infections through retail establishments and workplaces, they also increased infections through residences, as epidemiological models warned at the beginning of the pandemic. These results demonstrate the difficulty in determining the precise effects of policy changes as they are mediated through individual mobility and beliefs about the seriousness of the disease.
...
To summarize existing research, partisanship affects the unfolding of the pandemic by changing individuals' perceptions of the threat that COVID-19 posed. It is important to note, however, that the effect of partisanship is not limited to conservatives. A wave of protests following the death of George Floyd at the hands of police officers is an example of progressive movements for political change leading to risky public health behavior. Existing research suggests the protests have not had an adverse effect on COVID-19 infections [@protest2020], though it is again limited by the observational bias we describe later. While we know that individual partisanship and COVID-19 are correlated at the individual level, we still lack good knowledge of how and to what extent these associations extend to aggregate levels of infections in the population.
...
The stickiness of early conditions in the COVID-19 pandemic is becoming increasingly clear through analysis of the differential spread of early transmission. @blasius2020 shows that COVID-19 had a power law distribution in its early stages due to unchecked and unmitigated transmission. This highly skewed distribution resulted in very unequal levels of infections even in countries that share borders, such as China and Vietnam. While many expected the horrors of New York to appear in other states, the highly exponential level of growth in early clusters of COVID-19 did not replicate elsewhere with the same trajectory. As a result, seemingly random initial conditions, such as the fact that New York and New Jersey were states that voted against President Trump, produced a long-standing correlation between liberal-leaning areas and the level of infections despite lackluster efforts to control the pandemic in conservative states.

To establish this proposition, we will test the following two hypotheses:
  
  > H1: Higher levels of partisanship as measured by Trump vote share, Trump daily approval ratings and participation in racial justice protests are associated with reduced concern over COVID-19, increased within-state mobility and subsequent spread of the disease.
...
First, we propose that the number of infections is will almost certainty increase the number of tests as states try to stop the disease's spread via surveillance. Second, we can assume that a rising infection rate is associated with a higher ratio of positive results (reported cases) conditional on the number of tests, that is, COVID-19 is causing positive test results.
We model both of these observed indicators, tests and cases, jointly to simultaneously adjust for the infection rate's influence on both factors. It is this joint modeling that permits us to directly incorporate testing bias. In fact, our model learns about the infection rate from the level of tests rather than trying to discard this information.
...
We first show the association of mobility types with the infection rate. In Figure \@ref(fig:mobility) we show the marginal effect of a 1-SD increase in different types of Google mobility on the infection rate expressed as a fraction of a state's population. In line with the growing research on cellphone mobility and the epidemic, there are strong positive effects of some types of mobility on the spread of the disease, especially grocery stores, residential mobility and to a lesser extent workplace and retail mobility. Movement in parks and via transit has an estimated zero association with infections. While these results are somewhat surprising given that both residential and workplace mobility are very large, other results confirm with prior suspicions that outdoor activities like attending parks are relatively low-risk for COVID exposure. In fact, increased mobility parks is associated with reduced infections, probably because it substitutes for more high-risk types of mobility.
