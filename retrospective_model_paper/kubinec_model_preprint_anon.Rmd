...
The infection and subsequent hospitalization of United States president Donald Trump in early October of 2020 may have signaled the high water mark of a profound difference in how different political camps experienced and responded to the COVID-19 pandemic. While COVID-19 has been a difficult issue for many countries, in the United States profound disagreement over how to respond to the disease has exacerbated long-standing political divisions. In this paper, we provide clear and interpretable measures of the effect of partisanship and political mobilization--on both sides of the political divide--on infections. We also further decompose these results to better understand the channels by which partisanship effects COVID-19, such as by reducing social distancing or decreasing individual concern over the disease.

Although earlier studies have shown that associations between partisanship and COVID-19 exist at the individual level, it is very difficult to obtain inferences at a higher level of aggregation due to well-known biases in disease reporting. To address this critical problem, we present a Bayesian latent variable model to track COVID-19 spread in the United States. The utility of the model, compared to existing approaches, lies in its direct parameterization of the measurement bias stemming from incomplete testing for the disease in the United States. Rather than attempt to use statistical methods to eliminate the bias, we model this structure directly by jointly estimating the effect of covariates on COVID-19 cases and tests via the unobserved infection rate. This parameterization is much more precise than existing methods and employs relatively few assumptions while allowing us to make direct inferences on infections. 

We use this model to investigate the increasingly clear partisan division over the disease. We provide much more precise estimates of the association between partisanship and the disease's spread. We show that both Trump vote share in 2016 and racial justice protests are associated with rising infections, though the former much more so than the latter. To better explain these associations, we employ mediation analysis in our models to understand how much a particular factor suppresses the disease by reducing mobility (measured through cell phone signals) or by increasing people's concern about the disease (measured through daily polling) as opposed to an unmediated direct pathway. As a result, we show in this paper that the Trump effect on disease suppression does not come through reducing mobility or increasing concern over the pandemic, but rather appears to be an artifact of the arbitrary and highly path-dependent nature of the early spread of COVID-19.  

We aver based on this evidence that residents of pro-Trump states likely made the erroneous conclusion that the pandemic was a problem associated with more liberal areas of the country and would not affect them as strongly. As a result, people in pro-Trump states have tended to practice fewer social distancing behaviors and have displayed less concern over COVID-19, resulting in more infections despite favorable initial conditions. By contrast, racial justice protests are also associated with increased COVID-19 infections, though the effects are relatively smaller compared to other drivers of the pandemic. Nonetheless, it appears that political mobilization and identity on both the right and the left have undermined adherence to social distancing and other prudent measures to prevent infections.

In addition, we show that the effect of state-level suppression policies targeted at the epidemic varies significantly over the time. While some policies like stay-at-home orders seem to have an ability to suppress the epidemic consistently, other policies like business restrictions show declining effectiveness over time. As such, we believe that our focus on causal mediation analysis is crucial to understanding the intricate pathways by which beliefs, preferences and partisanship interact with state-level policies to produce the diverse patterns of the COVID-19 epidemic in the United States.
...
test_inf1 <- lapply(unique(real_data$cc), function(c) {
  ...
  country1s[,c] * real_data$lin_counter[real_data$cc==c,1] +
   country2s[,c] * real_data$lin_counter[real_data$cc==c,2] +
    country3s[,c] * real_data$lin_counter[real_data$cc==c,3] 
}) %>% do.call(rbind,.)
...
cases_pred <- lapply(sample(1:nrow(finding),100), function(i) {
  ...
  #mu_cases1 <-  exp(pcr_spec[i,] + finding[i,]*prop_infected_mat_scale[,i])
  ...
  tibble(out_pred=extraDistr::rbbinom(n=rep(1,nrow(prop_infected_mat_scale)),
                                      real_data$country_pop,
                                      prop_infected_mat_scale[,i]*phi[i,1],
                                      (1-prop_infected_mat_scale[,i])*phi[i,1]),
         out_pr1=prop_infected_mat_scale[,i],
         #out_pr2=mu_cases2,
         iter=i) %>% 
    mutate(key=1:n())
}) %>% bind_rows %>% 
...
e1 <- calc_sum %>% 
  ...
  annotate("text",x=rep(max(combined$month_day,2)),
           y=c(max_est-1300000,max_obs+300000),
           hjust=1,
           vjust=0,
           fontface="bold",
           size=2.7,
...
over_days <- lapply(seq(max(combined$month_day) - days(7), max(combined$month_day),by=1),
    ...
    over_states <- lapply(1:real_data$num_country, function(s) {
      ...
      alpha_est <- exp(alpha_test$`alpha_test[1]` + country_nonc1[,s] * lin_val[,1] +
                          country_nonc2[,s] * lin_val[,2] +
                           country_nonc3[,s] * lin_val[,3] +
                            test_base * this_infect1[[counter]][,s])
      
     tibble(estimate=alpha_est/(alpha_est + phi[,2]),
         state_num=s,
...
In contrast to other research, we find that social justice protests are positively associated with COVID-19 spread, though the effect is not particularly large. Furthermore, as we report cumulative marginal effects, it is unlikely that states experienced protests every day in the sample, suggesting that the reported effect is more of an upper bound for what most states experienced. There is some limited evidence, as @protest2020 suggest, that the positive effect of the protests was offset by reduced mobility by non-protesters as the indirect effect is weakly negative, though it is small (`r round(over_all_sum2$med_indirectmob_est[13]*100,3)`%). There is also a positive association via the fear pathway, suggesting that protest activity tended to reduce people's fear of COVID-19, and this may have led to increased infections independently of social contact.
